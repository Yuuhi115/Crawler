...
from comment_config_dialog import CommentConfigDialog
...
class BilibiliCrawlerFrame(wx.Frame):
    ...
    def create_ui(self):
        ...
        self.comment_config_btn = wx.Button(self.panel, label="评论设置")
        self.comment_config_btn.Bind(wx.EVT_BUTTON, self.on_comment_config)
        ...
        search_other_H_sizer_config.Add(self.comment_config_btn, 0, wx.ALL, 5)
        ...
        self.download_comment_btn = wx.Button(self.panel, label="获取评论")
        self.download_comment_btn.Bind(wx.EVT_BUTTON, self.on_download_comment)
        url_sizer.Add(self.download_comment_btn, 0, wx.ALL, 5)
    ...
    def on_comment_config(self, event):
        # 创建配置对话框
        dialog = CommentConfigDialog(self)
        dialog.ShowModal()
        dialog.Destroy()
    ...
    def on_download_comment(self, event):
        url = self.url_text.GetValue()
        if not url:
            wx.MessageBox("请输入视频链接", "提示", wx.OK | wx.ICON_WARNING)
            return
        ...
        download_thread = threading.Thread(target=self.comment_download_worker, args=(url,))
        download_thread.daemon = True
        download_thread.start()

    def comment_download_worker(self, url):
        try:
            run_comment_crawler(url)
            wx.MessageBox("评论下载完成", "提示", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            wx.MessageBox(f"评论下载过程中出现错误: {str(e)}", "错误", wx.OK | wx.ICON_ERROR)
